...
def main():
  ...
  p.add_argument('-e', '--exclude',
                 default=config.get('clangformat.exclude',
                                    'external'), metavar='<file>...',
                 help='exclude specified dir')
  ...
  filter_by_exclude(changed_lines, opts.exclude.lower().split(','))
  if opts.verbose >= 1:
...
def filter_by_exclude(dictionary, exclude_dir):
  exclude_dir = frozenset(exclude_dir)
  for filename in list(dictionary.keys()):
